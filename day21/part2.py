...
def rotate_based(s: str, a: str, undo: bool = False) -> str:
    if not undo:
        i = s.index(a)
        n = 1 + i + int(i >= 4)
        ...
        for cand_lst in itertools.permutations(s, len(s)):
            cand = ''.join(cand_lst)
            if rotate_based(cand, a) == s:
                return cand
    raise ValueError
    ...
    for line in lines:
        if 'swap' in line:
            if 'position' in line:
                ...
                word = swap_positions(word, int(y), int(x))
            elif 'letter' in line:
                _, _, a, _, _, b = line.split()
                word = swap_letters(word, b, a)
            else:
                raise ValueError
            ...
            if 'step' in line:
                ...
                *_, a = line.split()
                word = rotate_based(word, a, undo=True)
            else:
                raise ValueError
        elif 'move' in line:
            _, _, x, _, _, y = line.split()
            word = move(word, int(y), int(x))
        else:
            raise ValueError
    return word
...
@pytest.mark.parametrize(
    ...
    (
        (INPUT_S, WORD1, EXPECTED1),
        (INPUT2, WORD2, EXPECTED2),
    ),
)
